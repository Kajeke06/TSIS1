...
def display_game_results():
    ...
    # Запрос на выборку результатов из базы данных
    cur.execute("SELECT users.username, user_score.score FROM users INNER JOIN user_score ON users.user_id = user_score.user_id ORDER BY user_score.score DESC")
    results = cur.fetchall()

    # Вывод результатов в виде таблицы
    print("Имя пользователя\tБаллы")
    print("-----------------\t------")
    for row in results:
        print(f"{row[0]}\t\t\t{row[1]}")
...
# После завершения игры вызываем функцию для отображения результатов
